...
<body>
      ...
      <div class="col-8  col-md-5 table-responsive">
        <table class="content-table table">
          ...
          <tbody>
            ...
            WHERE inc.user_id = ' . $_SESSION['userId'] . ' AND cat.id = inc.income_category_assigned_to_user_id AND cat.user_id = inc.user_id AND date_of_income BETWEEN \'' . startMonthDate() . '\' AND \'' . endMonthDate() . '\' ORDER BY inc.amount DESC;';
    ...
    <div class="row justify-content-center align-items-center">
      ...
      <div class="col-8  col-md-5 table-responsive">
        <table class="content-table table">
          ...
          WHERE ex.user_id = ' . $_SESSION['userId'] . ' AND cat.id = ex.expense_category_assigned_to_user_id AND cat.user_id = ex.user_id AND date_of_expense BETWEEN \'' . startMonthDate() . '\' AND \'' . endMonthDate() . '\' ORDER BY ex.amount DESC;';
  ...
  incomes_category_assigned_to_users AS cat,incomes AS inc
  WHERE inc.user_id = ' . $_SESSION['userId'] . ' AND cat.id = inc.income_category_assigned_to_user_id AND cat.user_id = inc.user_id AND date_of_income BETWEEN \'' . startMonthDate() . '\' AND \'' . endMonthDate() . '\' GROUP BY cat.name;';
  ...
  WHERE inc.user_id = ' . $_SESSION['userId'] . ' AND cat.id = inc.expense_category_assigned_to_user_id AND cat.user_id = inc.user_id AND date_of_expense BETWEEN \'' . startMonthDate() . '\' AND \'' . endMonthDate() . '\' GROUP BY cat.name;';
